...
XAuctionBoard.refreshUI = function()
    XAuctionBoard.mainFrame.title:SetText('拍卖记录BTMAD (' ..
        (displayPageNo + 1) ..
...
local function initUI()
    local mainFrame = XUI.createFrame("XAuctionBoardMainFrame", 320, 430)
    ...
    for i = 1, displayPageSize do
        ...
        infoButton:SetScript("OnClick", function()
            ...
            XUIInputDialog.show('XAuctionBoard_Craft', function(input)
...
XAuctionBoard.addItem = function(itemName)
    local existed = false
...
XAutoAuction.registerEventCallback('XAuctionBoard', 'ADDON_LOADED', function()
    initUI()
    XAuctionBoard.refreshUI()
end)

XAutoAuction.registerEventCallback('XAuctionBoard', 'CHAT_MSG_SYSTEM', function(self, event, text, context)
    if XUtils.stringStartsWith(text, '你拍卖的') and XUtils.stringEndsWith(text, '已经售出。') then
        ...
        XAuctionBoard.addItem(str)
        XAuctionBoard.refreshUI()
    end
end)

XAutoAuction.registerEventCallback('XAuctionBoard', 'AUCTION_HOUSE_SHOW', function(self, event, text, context)
    if XAuctionBoard.mainFrame ~= nil then XAuctionBoard.mainFrame:Show() end
end)

XAutoAuction.registerEventCallback('XAuctionBoard', 'AUCTION_HOUSE_CLOSED', function(self, event, text, context)
    if XAuctionBoard.mainFrame ~= nil then XAuctionBoard.mainFrame:Hide() end
end)

SlashCmdList["XAUCTIONBOARD"] = function()
    if XAuctionBoard.mainFrame ~= nil then
        if XAuctionBoard.mainFrame:IsVisible() then
            XAuctionBoard.mainFrame:Hide()
        else
            XAuctionBoard.mainFrame:Show()
        end
    end
end
